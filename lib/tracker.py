...
class Tracker(object):
    ...
    def step(self, results, public_det=None):
        ...
        track_size = np.array([((track['bbox'][2] - track['bbox'][0]) * (track['bbox'][3] - track['bbox'][1]))
                               for track in self.tracks], np.float32)  # 计算所有轨迹窗口面积
        track_cat = np.array([track['class'] for track in self.tracks], np.int32)  # 计算所有类别
        item_size = np.array([((item['bbox'][2] - item['bbox'][0]) * (item['bbox'][3] - item['bbox'][1]))
                              for item in results], np.float32)  # 计算检测结果所有窗口面积
        item_cat = np.array([item['class'] for item in results], np.int32)  # 计算检测结果所有类别
        tracks = np.array([pre_det['ct'] for pre_det in self.tracks], np.float32)  # 所有轨迹中心
        dist = (((tracks.reshape(1, -1, 2) - dets.reshape(-1, 1, 2)) ** 2).sum(axis=2))  # 计算欧式距离
